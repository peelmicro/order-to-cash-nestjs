...
//
// IDENTICAL shape to apps/orders' and apps/fulfillment's outbox.schema.ts —
// divergence between the three services' outboxes (orders/fulfillment/
// billing) would be a defect; copied verbatim per db_billing's task prompt.
// No `causation_id` column here either — that decision belongs to feature 14
// (outbox_and_idempotency) and must land in all three DBs together (see
// progress/review_db_orders.md advisory #1).
...
export const outbox = mysqlTable(
  'outbox',
  {
    ...
    // `<aggregate>.<fact>.v<n>`, e.g. "invoice.issued.v1".
