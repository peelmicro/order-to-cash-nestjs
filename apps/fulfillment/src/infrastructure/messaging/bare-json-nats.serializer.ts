// The reply half of the wire finding (design.md §6.3): `@nestjs/microservices`'s
// default `NatsRecordSerializer` wraps every reply in the Nest packet
// `{ response, isDisposed, id, err }` — not the bare payload Orders'
// adapters decode. `BareJsonNatsSerializer` emits `packet.response` ALONE
// when present; when `packet.err` is set (the only source is Nest's own
// `NO_MESSAGE_HANDLER`, since `StockController` never throws) it emits a
// bare `RpcError` `INTERNAL_ERROR`. Nothing else from the packet reaches
// the wire.
