...
// transaction (design.md §7, §8.1). Called by `DrizzleStockItemRepository.saveAll`
// after it drains each item's `pullDomainEvents()` — never by a handler
// directly, so a dual-write (aggregate row without its facts, or vice
// versa) cannot happen by omission.
