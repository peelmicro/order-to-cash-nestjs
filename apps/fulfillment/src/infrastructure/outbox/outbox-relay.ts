...
// The relay's core — design.md §5.1, §5.2, §8.1. A PLAIN CLASS, no NestJS
// decorator: `runOnce()` is directly callable from a test without a Nest
// application context. `OrdersDb` -> `FulfillmentDb` is the ONE edit
// against the Orders original (design.md §8.1) — the schema itself is
// already byte-identical (OI11), which is what makes this SQL correct
// without change. NOT parity-guarded yet (design.md §8.3) — feature 19
// owns arming that guard once the canonical is made service-neutral.
import { asc, inArray, isNull } from 'drizzle-orm';
import type { Clock } from '../../application/ports/clock.port';
import type { FactPublisher, PublishableFact } from '../../application/ports/fact-publisher.port';
import type { FulfillmentDb } from '../persistence/client';
...
/** Raised when `publisher.publish(...)` has not settled within `config.publishTimeoutMs`. Handled the same way any other publish failure is (§5.3, OI8): the batch is left unstamped and retried, unchanged, on the next poll. */
