// COPY OF — apps/orders/src/infrastructure/outbox/kafka-fact-publisher.ts
// The relay's kafkajs producer adapter (design.md §5.3, §8.1). kafkajs
// directly, not `@nestjs/microservices` `ClientKafka`: the relay needs
// explicit control of the partition key, the idempotent-producer flags and
// the acknowledgement point, all of which `ClientKafka` abstracts away.
// Default topic `ORDERS_FACTS_TOPIC` -> `FULFILLMENT_FACTS_TOPIC` — the ONE
// edit against the Orders original.
//
...
export class KafkaFactPublisher implements FactPublisher {
  ...
  constructor(client: KafkaClientLike, private readonly topic: string = FULFILLMENT_FACTS_TOPIC) {
