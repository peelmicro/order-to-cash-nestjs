...
**Position: Phase 7 of 25 complete, Phase 8 underway — 15 of 39 features done** (the backlog grew by one: `requestId` idempotent replay was scoped out of order acceptance honestly, as a tracked `sdd: true` feature, rather than left silently absent).
...
| 8 | Orders service — `orders_aggregate` ✅, `outbox_and_idempotency` ✅, `orders_acceptance` ✅ (first NATS entry point); saga orchestrator next | 🚧 |
